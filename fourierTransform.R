...
# fourier series works only on linear systems (no saturation (overflow) from some level like below )
wave.4 <- wave.3
wave.4[wave.3>0.5] <- 0.5
plot(xs,wave.4,type="l",ylim=c(-1.25,1.25)); title("overflowed, non-linear complex wave"); abline(h=0,lty=3)


#Also, the Fourier Series only holds if the waves are periodic, ie, they have a repeating pattern (non periodic waves are dealt by the Fourier Transform)


# Here’s a R function for plotting trajectories given a fourier series:
# w - omega ; w = 2*pi*f0; ts - sampling time
...
# An eg
plot.fourier(function(t,w) {sin(w*t)}, 5, ts=seq(0,1,1/100)) 



# function for plotting fourier series, useful for other examples
...
# alternative representation using the identity: sin(a+b)=sin(a)cos(b)+cos(a)sin(b)

#The Fourier Transform (FT) is a generalization to solve for non-periodic waves. The FT assumes that the finite analyzed 
#segment corresponds to one period of an infinitely extended periodic signal.

# The Fourier Transform sees every trajectory (aka time signal, aka signal) as a set of circular motions.
# 
# Given a trajectory the fourier transform (FT) breaks it into a set of related cycles that describes it. Each cycle has a strength, a delay and a speed. These cycles 
# are easier to handle, ie, compare, modify, simplify, and if needed, they can be used to reconstruct the original trajectory.
# 
#
...
#   
#   the Discrete Fourier Transform (DFT) which requires O(n^2)
# 
#   the Fast Fourier Transform (FFT) which requires O(n.log(n))
#
# There’s a R function called fft() that computes the FFT.

# eg of stationary and an increasing trajectory:

# R returns a sequence complex numbers
fft(c(1,1,1,1)) / 4  # to normalize
fft(1:4) / 4  


# Cycles are in freq domain - represents strength of each harmonic 

# cycles 0 0 1 -> odpowiada kolejnym czestotliwosciom (0, 1, 2) Hz
# cycles 0 1 2 -> zlozenie sin 1Hz o sile 1  oraz 2Hz o sile

#Function to convert complex numbers (cycles), to pairs strength:delay (in degrees).
...
# Usually we want to know what should the cycles be so that their combine strengths result on a given known trajectory? That is what DFT/FFT computes.

# A Fourier Transform converts a wave from the time domain into the frequency domain.
# An inverse Fourier Transform (IFT) converts the frequency domain components back into the original time wave
...
# a vector with the amount of frequencies k in the signal (X.k) - freq domain
# x.n - in time domain
...
fOut<-fft(c(1:4))
fft(fOut, inverse = T) / length(fOut)

 
# Here’s two useful functions:
# 
# plot.frequency.spectrum() plot a frequency spectrum of a given Xk
# 
...
################################################################
#R packages that produce this type of frequency plots:
...
# look at the end of script -> HERE tag
...
plot.harmonic(X.k,0,ts,acq.freq,"yellow")
plot.harmonic(X.k,1,ts,acq.freq,"red")
plot.harmonic(X.k,2,ts,acq.freq,"green")
plot.harmonic(X.k,3,ts,acq.freq,"blue")

# The highest meaningful sin wave frequency – after the fft-analysis of the original waveform signal – is at half the data acquisition frequency, because our input signal
# is composed of real values (ie, trajectory has no imaginary parts). The last useful bin is at acq.freq/2. The Nyquist Frequency is half the sampling rate. The Nyquist frequency
# is the maximum frequency that can be detected for a given sampling rate. This is because in order to measure a wave you need at least one trough and one peak to identify it.
# 
